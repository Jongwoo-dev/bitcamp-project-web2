...
4) 자바스크립트 라이브러리를 추가한다.
   ...
   - 다운로드 받은 라이브러리는 npm을 실행시킨 폴더에 node_modules라는 폴더를 만들고,
     그 아래에 다운로드 받는다.
5) 자동으로 모든 의존 라이브러리 다운로드 받기
   > npm install
   - 라이브러리 이름을 지정하지 않으면, 실행 폴더에서 package.json을 찾는다.
   - package.json에 등록된 의존 라이브러리를 자동으로 받는다ㅡ.
...
1) CDN(Content Delivery Network)  방식
  ...
  - 신뢰할만한 서버에 올려져있는 라이브러리 파일의 링크를 건다.
    - 즉 직접 자신들의 웹 서버에 해당 파일을 놓지 않는다.
    - 최종 사용자는 같은 URL로부터 다운로드 받은 파일을 재사용할 수 있다.
    - 사이트마다 같은 URL의 파일을 사용한다면 다시 다운받지 않아도 되기 때문에 페이지 로딩 속도가 높아진다.
  - 단점?
...
## 3.3 - 자바스크립트 라이브러리 압축하기
  ...
  - 자바스크립트는 zip 형식으로 압축하면 안된다. 왜? 소스 코드를 직접 인터프리팅하기 때문이다. 압축되어 있으면 해석이 불가능하다.
...
- JSP 파일을 WEB-INF 밑으로 옮긴다.
  ...
  - WEB-INF 폴더는 웹애플리케이션의 설정파일을 두는 폴더이기 때문이다.
- 작업
...
- Service 객체의 구현체 생성
  - StudentServiceImpl, MaganerServiceImpl, TeacherServiceImpl, AuthServiceImpl
...
- DMBS 접속 정보를 담은 프로퍼티 파일 준비
  - resources/conf/jdbc.properties 준비
- SQL Mapper 파일 준비
...
- 스프링 필터와 리스터를 사용하려면 spring-webmvc 라이브러리를 추가해야 한다.
  - build.gradle 변경
...
## 1.3 - 로그인 할 떄 사용자의 유형을 지정하고, 로그인 사용자의 사진 출력
- 로그인 폼에 사용자 유형 선택 추가
...
- 강사 사진 업로드
  ...
  - TeacherAddServlet, ManagerUpdateServlet 클래스 변경
    - fileupload 라이브러리의 클래스를 사용하여 멀티파트 데이터 처리
...
- 다른 서블릿이 사용할 객체를 DAO 객체를 준비시키는 서블릿을 만든다.
